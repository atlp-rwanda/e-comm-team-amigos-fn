...
export const clearCart = () => {
	return async (dispatch) => {
		...
		try {
			const token = localStorage.getItem("token");
			const response = await fetch("https://e-comm-team-amigos-bn-project.onrender.com/cart/clean-up-cart", {
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${token}`,
...
export const removeItemCart = (productId) => {
	return async (dispatch) => {
		...
		try {
			const token = localStorage.getItem("token");
			const response = await fetch(`https://e-comm-team-amigos-bn-project.onrender.com/cart/delete-cart/${productId}`, {
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${token}`,
			...
			dispatch({ type: REMOVE_ONE_ITEM_CART_ERROR, payload: error.message });
			dispatch({
				type: REMOVE_ONE_ITEM_CART,
				payload:false,
			});
